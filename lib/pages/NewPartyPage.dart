...
class AddPartyPage extends StatefulWidget {
  ...
  const AddPartyPage({Key? key, this.partyToEdit, this.editIndex}) : super(key: key);
...
Future<void> savePartyToPrefs() async {
 ...
 Future<void> deleteParty() async {
  print('=== DELETE FUNCTION STARTED ==='); // New debug line
  
  if (widget.editIndex == null) {
    print('Edit index is null - aborting'); // New debug line
    return;
  }

  print('Attempting to delete party at index: ${widget.editIndex}');
  
  try {
    final prefs = await SharedPreferences.getInstance();
    print('SharedPreferences instance obtained'); // New debug line
    
    List<String> existing = prefs.getStringList('parties') ?? [];
    print('Current parties list length: ${existing.length}');
    
    if (widget.editIndex! < existing.length) {
      print('Index is valid - proceeding with deletion'); // New debug line
      ...
      print('Index out of bounds - index: ${widget.editIndex}, length: ${existing.length}');
    }
  } catch (e) {
    print('Error during deletion: $e'); // New error handling
  }
}
