...
const Map<String, String> switchPreferenceKeys = {
  ...
  "4.1 Send bill to customer on whatsapp":"4.1 Send bill to customer on whatsapp",
  "4.2 Send bill to owner on whatsapp":"4.2 Send bill to owner on whatsapp",
  "4.3 choose days for reminder Message":"4.3 choose days for reminder Message"
...
class _billSettingsPageState extends State<billSettingsPage> {
  String selectedStyle = 'Restaurant With Image Style';
...
Future<Map<String, bool>> loadSwitchValues() async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: const [
            Text("Ezo", style: TextStyle(fontSize: 20)),
            Text("EZO v36.35 | 285a3977", style: TextStyle(fontSize: 12)),
          ],
        ),
      ),
      body: ListView(
        ...
        children: [
          ...
          toggleTile(
              "2.7 Sort by Item Code (Item Selector)",
              "",
            ),
            toggleTile(
   ...
   const SizedBox(height: 12),
            ...
            buildDropdownTile(
                "2.13 Item Category List Width (Item Selector)", "20%"),
            // buildSwitchTile(
            ...
            toggleTile(
              "2.14 Item Price - Multiple Options",
              "Enabled",
            ),
            toggleTile(
               "2.15 Alpha Numeric Barcodes",
              "Enabled",
            ),
            toggleTile(
               "2.16 Round Off Total Amount",
              "Enabled",
            ),

             toggleTile(
               "2.17 Always Show Previous Balance",
              "",
            ),
           buildInlineInputTile("   2.18 Service Charge (%)", "e.g. 5.00"),

            //2.18 is textbox
             toggleTile(
               "2.19 Party wise Item price",
              "",
            ),
               toggleTile(
               "2.20 Hide out of stock items",
              "",
            ),
             toggleTile(
               "2.21 Bill Quick save",
              "",
            ),

             toggleTile(
               "2.22 show current stock in bill",
              "",
            ),
          ...
          const SizedBox(height: 20),
            ...
            // 🔵 Section 2: App Settings
              ...
              toggleTile(
               "3.1 Landscape mode ",
              "useful for tablet/big screen",
            ),

            toggleTile(
               "3.2 Powered by Orbipay",
              "Print tagline on bill",
            ),
              toggleTile(
               "3.3 domain by Orbipay",
              "Print tagline on bill",
            ),
             toggleTile(
               "3.4 Bill Amount Sound",
              "Print tagline on bill",
            ),
          
            toggleTile(
               "3.5 Pending KOT Sound",
              "it play sound",
            ),
            toggleTile(
             ...
             toggleTile(
               "3.7 Configure App Colors",
              "Choose your fav color",
            ),
            toggleTile(
               "3.8 Cut off day status",
              "",
            ),
            toggleTile(
               "3.9 Item selection Sound",
              "",
            ),
            const Text(
                "4.Other Settings",
            ...
            toggleTile(
               "4.2 Send bill to owner on whatsapp",
              "",
            ),
             toggleTile(
...
Widget itemSelector() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("2.1 Item Selector Style",
              ...
              child: Container(
                ...
                child: ListTile(
                  leading: CircleAvatar(
                    radius: 8,
                    backgroundColor:
                        selected ? Color(0xFF3F35F4) : Colors.grey.shade400,
                  ),
                  title: Text(style),
                ),
              ),
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("2.23 Select item filter priority",
            style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        Column(
          children: styles.map((style) {
...
Widget buildInlineInputTile(String label, String hint) {
  return Padding(
